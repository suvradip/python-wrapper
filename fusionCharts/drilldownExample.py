...
def fc_db(request):
	...
	for key in Country.objects.all():
	  ...
	  # It is used to specify the URL of the page to which the user is redirected to when a data plot is clicked.
	  data['link'] = 'DrillDown?code='+ key.Code
...
# The `fc_drilldown` function is defined to load data from a `City` Model. 
# This data will be converted to JSON and the chart will be rendered.

def fc_drilldown(request):
	# Get the country code from the GET parameter
	codeName = request.GET['code']
	# Chart data is passed to the `dataSource` parameter, as dict, in the form of key-value pairs.
	dataSource = {}
	dataSource['chart'] = { 
		"caption" : "Top 10 Most Populous Countries",
	...
	for key in City.objects.all().filter(CountryCode=codeName):
	  data = {}
	  data['label'] = key.Name
	  data['value'] = key.Population
	  dataSource['data'].append(data)
